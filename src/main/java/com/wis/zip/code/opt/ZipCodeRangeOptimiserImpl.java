...
public class ZipCodeRangeOptimiserImpl implements ZipCodeRangeOptimiser {

	/**
	 * This method is used to get optimized Zip code boundaries, expected input
	 * example: [94133,94133] [94200,94299] [94600,94699]
	 * 
	 ...
	 */
	@Override
	public String getOptimizeZipBoundary(final String zipCodeboundarySets) throws ZipCodeServiceException {
		...
		final String[] arrayOfZipcodeBoundaries = zipCodeboundarySets.split("]");
		final List<ZipCode> zipList = new ArrayList<>();
		...
		zipList.sort((zipcode1, zipcode2) -> new Integer(zipcode1.getLowerLimit()).compareTo(zipcode2.getLowerLimit()));
		List<ZipCode> zipcodeOutputList = getOptimizeValues(zipList);
		StringBuilder outputBuilder = new StringBuilder();
		zipcodeOutputList.stream().forEach(zipCodeOutput -> {
	...
	private boolean isInputValid(String zipCodeBoundarySets) {
		...
		if (zipCodeBoundarySets == null) {
			...
			for (int i = 12; i < zipCodeBoundarySets.length(); i = i + 13) {
				...
				if (zipCodeBoundarySets.length() != i + 1
						&& !zipCodeBoundarySets.subSequence(i + 1, i + 2).equals("[")) {
					isInputValid = false;
					break;
				}
			}
	...
	public static List<ZipCode> getOptimizeValues(final List<ZipCode> zipList) {
		final List<ZipCode> optList = new ArrayList<>();
		if (null != zipList && !zipList.isEmpty()) {
			ZipCode zipPrev = zipList.get(0);
			for (int i = 1; i < zipList.size(); i++) {
				ZipCode zipNext = zipList.get(i);
				/**
				 * There could be the below scenarios 1. if previous zip code lower limit is
				 * less than or equal to the next zip code lower limit and if previous zip code
				 * upper limit is greater than or equal to the next zip code then this case need
				 * to ignore to store into the zip code, since this will cover win prev zip code
				 * list 2. If prevZip code lower limit is less than the next zip code lower
				 * limit and if the prevZip upper limit is greater than next zip code of lower
				 * limit then replace the upper limit of previous zip code and continue 3. If
				 * the prevZip lower limit is less than or equal to next zip of lower limit and
				 * if the prevZip of upper limit is less than next zip of lower limit then add
				 * prevZip into list assign prevZip with next zip
				 */
